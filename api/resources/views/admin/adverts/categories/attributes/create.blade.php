...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.adverts.categories.attributes.store', $category) }}">
        ...
        <div class="form-group">
            ...
            <input id="sort" type="text" class="form-control{{ $errors->has('sort') ? ' is-invalid' : '' }}"
        ...
        <div class="form-group">
            ...
            <select id="type" class="form-control{{ $errors->has('type') ? ' is-invalid' : '' }}" name="type">
                @foreach ($types as $type => $label)
                    <option value="{{ $type }}"{{ $type == old('type') ? ' selected' : '' }}>{{ $label }}
                    </option>
                @endforeach;
            </select>
            @if ($errors->has('type'))
        ...
        <div class="form-group">
            ...
            <div class="checkbox">
                <label>
                    <input type="checkbox" name="required" {{ old('required') ? 'checked' : '' }}> Обязательное
                </label>
            </div>
