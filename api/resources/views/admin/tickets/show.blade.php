...
@section('content')
    ...
    <div class="d-flex flex-row mb-3">
        ...
        <a href="{{ route('admin.tickets.edit', $ticket) }}" class="btn btn-primary me-1">Изменить</a>

        @if ($ticket->isOpen())
            <form method="POST" action="{{ route('admin.tickets.approve', $ticket) }}" class="me-1">
                @csrf
                <button class="btn btn-success">Принять в работу</button>
            </form>
        @endif

        @if (!$ticket->isClosed())
            <form method="POST" action="{{ route('admin.tickets.close', $ticket) }}" class="me-1">
                @csrf
        ...
        @if ($ticket->isClosed())
            <form method="POST" action="{{ route('admin.tickets.reopen', $ticket) }}" class="me-1">
                @csrf
                <button class="btn btn-success">Открыть</button>
            </form>
        @endif

        <form method="POST" action="{{ route('admin.tickets.destroy', $ticket) }}" class="me-1">
            @csrf
            @method('DELETE')
